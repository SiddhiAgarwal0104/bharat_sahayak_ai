...
import {
  ...
  IndianRupee, BookOpen, ArrowRight,
  Volume2, VolumeX, Square
} from "lucide-react"

const CATEGORY_META = {
  health:      { text: "text-red-800",   dot: "bg-red-500"   },
  agriculture: { text: "text-green-800", dot: "bg-green-500" },
  pension:     { text: "text-amber-800", dot: "bg-amber-500" },
  women:       { text: "text-pink-800",  dot: "bg-pink-500"  },
  education:   { text: "text-blue-800",  dot: "bg-blue-500"  },
  other:       { text: "text-gray-700",  dot: "bg-gray-400"  },
}

const LANG_CODE_MAP = {
  Hindi: "hi", English: "en", Tamil: "ta",
  Telugu: "te", Bengali: "bn", Marathi: "mr",
  hi: "hi", en: "en", ta: "ta", te: "te", bn: "bn", mr: "mr",
}
...
const normalizeLang = function(pref) {
...
function checkEligibility(user, criteria) {
  ...
  if (criteria.max_income)
    checks.push({ label: "Income below Rs." + criteria.max_income.toLocaleString(), pass: Number(user.annual_income) <= criteria.max_income })
  if (criteria.gender)
...
function useVoicePlayer(schemeId, lang) {
  ...
  var play = async function() {
    ...
    try {
      ...
      var url = backendUrl + "/schemes/" + schemeId + "/voice?lang=" + lang
      ...
      audio.onerror = function() { setState("error") }
      await audio.play()
      setState("playing")
    } catch (e) {
...
function VoiceButton(props) {
  ...
  var lang = props.lang
  var result = useVoicePlayer(schemeId, lang)
  var state = result.state
  var play = result.play
  var langLabel = LANG_LABEL[lang] || "Hindi"
  ...
  var Icon = Volume2
  if (state === "playing") Icon = Square
  if (state === "loading") Icon = Loader2
  if (state === "error")   Icon = VolumeX

  var colorClass = "bg-white border-green-200 text-green-700 hover:bg-green-50"
  if (state === "error")   colorClass = "bg-red-50 border-red-200 text-red-700 hover:bg-red-100"
  if (state === "playing") colorClass = "bg-green-600 border-green-600 text-white hover:bg-green-700"
  ...
  return (
    ...
    >
      ...
      <span className="whitespace-nowrap">{labelMap[state]}</span>
    </button>
  )
}
...
export default function SchemeDetail() {
  ...
  var id = params.id
  var authResult = useAuth()
  var user = authResult.user
  var navigate = useNavigate()
  var lang = normalizeLang(user ? user.language_pref : "en")

  var schemeArr = useState(null)
  var scheme = schemeArr[0]
  var setScheme = schemeArr[1]
  ...
  var translatedArr = useState(null)
  var translated = translatedArr[0]
  var setTranslated = translatedArr[1]

  var translatingArr = useState(false)
  ...
  useEffect(function() {
    getScheme(id)
      .then(function(r) { setScheme(r.data) })
      .catch(function() { setError("Could not load scheme. Please try again.") })
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-red-50 border border-red-200 rounded-3xl p-10 text-center">
            ...
            <button
              ...
              className="px-6 py-3 bg-white border-2 border-red-200 text-red-700 rounded-xl font-bold hover:bg-red-50 transition-colors"
            >
              Go Back
            </button>
          </div>
  ...
  var cat = (scheme.category || "other").toLowerCase()
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] overflow-hidden shadow-[0_20px_60px_-15px_rgba(22,163,74,0.15)] border border-white">
          ...
          <div className="relative h-[260px] overflow-hidden">
            ...
            <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between">
              <div>
                <span className={"inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest bg-white/95 backdrop-blur-md mb-3 " + meta.text}>
          ...
          <div className="p-8 md:p-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button
                onClick={function() { navigate("/scheme/" + scheme.id + "/form") }}
                className="flex-1 flex items-center justify-center gap-3 px-8 py-4 bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600 text-white rounded-2xl font-bold text-lg transition-all shadow-[0_8px_20px_rgba(22,163,74,0.3)] hover:-translate-y-0.5 active:scale-95"
              >
              ...
              <div className="space-y-3">
                {checks.map(function(c, i) {
                  return (
                    <div
                      key={i}
                      className={"flex items-center gap-3 p-3 rounded-xl border " + (c.pass ? "bg-green-50 border-green-100" : "bg-red-50 border-red-100")}
                    >
                      {c.pass
